...
function ss2gs3(msg_type,SS_LOCATION_ONE)
    %LMB3声明为全局变量，能够保存变量值并被其他函数调用，用于存储用户站相关信息
    global LMB3;
    ...
    if strcmp(msg_type,'00000000')
        ...
        %信关站启用位置更新线程
    %     %LMB3声明为静态变量，在函数的多次调用中能够保存存储的数据，用于存储用户站相关信息
    %     persistent LMB3;

        %第一次调用ss2gs3()函数时，即第一次有用户站上报位置信息时，LMB是空的
        %则令LMB3 = SS_LOCATION_ONE以记录第一条用户站位置信息
        if isempty(LMB3)
            LMB3 = [SS_LOCATION_ONE,0];
            ...
            if ~ismember(SS_LOCATION_ONE(6),LMB3(:,6))
                LMB3 = [LMB3;SS_LOCATION_ONE,0];
                %通知中心站，形参1是消息类型，形参2是用户站编号，形参3是信关站编号
                gs2gdb('00000001',SS_LOCATION_ONE(6),SS_LOCATION_ONE(5));
            else
                %若本地LMB含当前用户站，说明是非跨信关站的位置更新，通知ss
                %更新已有用户站的信息，注意index是一个只含一个“1”其余全是“0”的数组
                index = LMB3(:,6)==SS_LOCATION_ONE(6);
                LMB3(index,:) = [SS_LOCATION_ONE,0];
                %通知ss位置更新的结果
