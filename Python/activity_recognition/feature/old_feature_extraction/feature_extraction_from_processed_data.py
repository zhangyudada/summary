...
def feature_extraction(load_csv, write_csv):
    ...
    for num in range(df.shape[0]):
        # there are some problem exist when calculate logpower_x
        # AttributeError: 'numpy.float64' object has no attribute 'log'
        # and cor_xy
        # AttributeError: 'float' object has no attribute 'shape'
        # so it is a good idea to change the dtype of x,y,z at the beginnig
        # rather than to change the dtype of relative varibles when needed
        # add .astype(float) to the end of x,y,z
        
#        x = df.loc[num][1:78]
#        y = df.loc[num][78:155]
#        z = df.loc[num][155:232]
        
        x = df.loc[num][1:78].astype(float)
        y = df.loc[num][78:155].astype(float)
        z = df.loc[num][155:232].astype(float)
...
#        # .astype(float) method to solve following problem
#        # logpower_x = sum(np.log(rpls_x ** 2))
#        # AttributeError: 'numpy.float64' object has no attribute 'log'
#        rpls_x = np.where(x == 0, 10**-10, x).astype(float) 
#        rpls_y = np.where(y == 0, 10**-10, y).astype(float) 
#        rpls_z = np.where(z == 0, 10**-10, z).astype(float)
...
#        # c = cov(x, y, rowvar)
#        # AttributeError: 'float' object has no attribute 'shape'
#        cor_xy = np.corrcoef(np.array([x.astype(float),y.astype(float)]))[0,1]
#        cor_yz = np.corrcoef(np.array([y.astype(float),z.astype(float)]))[0,1]
#        cor_zx = np.corrcoef(np.array([z.astype(float),x.astype(float)]))[0,1]
    ...
    for label in labels:
        ...
        temp_df.to_csv('./data/processed_data/all/features_from_processed_data/{}.csv'.format(label), index=False)

load_csv = './data/processed_data/all/all_data.csv'
write_csv = './data/processed_data/all/features_from_processed_data/all_data.csv'
